...
const instancingExVertexShaderSource: string = `#version 300 es
...
const instancingExFragmentShaderSource: string = `#version 300 es
...
function generateCube(size: number) {
...
function initInstancingEx() {
    ...
    const vertexShader: WebGLShader = createShader(gl, gl.VERTEX_SHADER, instancingExVertexShaderSource);
    const fragmentShader: WebGLShader = createShader(gl, gl.FRAGMENT_SHADER, instancingExFragmentShaderSource);
    // programingObj 생성
